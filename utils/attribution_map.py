...
plot_attribution_grid     : save a multi-row figure for one group of examples
        ...
        for case in tqdm(cases, desc=f"  {group_name}", leave=False):
            try:
                ...
                if "out of memory" in str(exc).lower():
                    ...
                    logger.warning(
                        f"CUDA OOM — skipped image in group '{group_name}' "
                        f"(label={case['label']:.0f} W/m²).  "
                        f"GPU memory cleared; continuing."
                    )
                    maps.append(None)
                else:
                    raise
...
def save_attribution_stats(group_attr_data, save_dir, top_n=40):
    ...
    for group_name, cases_maps in group_attr_data.items():
        for img_idx, (case, attr_map) in enumerate(cases_maps):
            if attr_map is None:
                logger.warning(f"save_attribution_stats: skipping OOM image [{group_name}#{img_idx}]")
                continue
...
def _make_diverging_overlay(orig_uint8, attr_map_norm, alpha=0.75, gamma=0.4):
    """Blend a signed [-1,1] attribution map onto original.

    Red tint for positive (energy boosters), blue tint for negative
    (energy blockers).  A power-law (gamma < 1) is applied to boost
    visibility of subtle attribution regions.
    ...
    gamma         : float — power-law exponent (< 1 amplifies subtle values)
    ...
    pos = np.power(np.clip(attr_map_norm, 0, 1), gamma)
    neg = np.power(np.clip(-attr_map_norm, 0, 1), gamma)
...
def plot_attribution_grid(
    ...
    # Group-level scale — actual data range, no per-image normalization
    if is_signed:
        absmax = max(float(np.max(np.abs(a))) for a in all_attr_maps)
        if absmax == 0:
            absmax = 1.0
        vmin, vmax = -absmax, absmax
        cmap = "RdBu_r"
    else:
        vmax = max(float(np.max(a)) for a in all_attr_maps)
        if vmax == 0:
            vmax = 1.0
        vmin = 0
        cmap = "Reds"
    ...
    for i, (case, attr_map) in enumerate(zip(cases, all_attr_maps)):
        ...
        # Overlay requires values in [0, 1] for RGB blending — divide by group max only
        if is_signed:
            overlay_map = np.clip(attr_map / absmax, 0, 1)   # positive side only for red tint
        else:
            overlay_map = np.clip(attr_map / vmax, 0, 1)

        # Denormalize image and build overlay (red tint)
        orig_uint8 = denormalize_image(img_tensor, normalize_mean, normalize_std)
        overlay = _make_overlay(orig_uint8, overlay_map, alpha=alpha)
        ...
        axes[i, 1].imshow(attr_map, cmap=cmap, vmin=vmin, vmax=vmax)
        if i == 0:
    ...
    # Colorbar legend — actual data range
    ...
    if is_signed:
        ...
        cbar.set_ticklabels([f"{vmin:.4f}  (blocks)", "0", f"{vmax:.4f}  (boosts)"])
    else:
        cbar.set_ticks([0, vmax])
        cbar.set_ticklabels(["0", f"{vmax:.4f}  (high)"])
    cbar.ax.tick_params(labelsize=9)

    caption = (
        f"Raw {method_name} attribution (no normalization).  "
        f"Group scale: {vmin:.4f} … {vmax:.4f}.  "
        f"Col 1: Original.  Col 2: Attribution map.  Col 3: Red-tint overlay (÷group max for blending only)."
    )
...
def plot_ig_signed_grid(
    ...
    No normalization or gamma correction is applied to the raw attribution
    values.  The color scale is set to the actual data range across all images
    in the group (group-level absmax), so all rows share a consistent scale.
    The overlay requires values in [-1, 1] for RGB blending math; the only
    transformation applied is dividing by that group absmax.
    ...
    precomputed_maps : list[np.ndarray] or None
        Raw signed attribution maps (one per case).  If provided, ``attr_fn``
        is not called.
    """
    n = len(cases)
    ...
    # Group-level absolute maximum — shared scale across all rows, no per-image distortion
    absmax = max(float(np.max(np.abs(m))) for m in all_attr_maps)
    if absmax == 0:
        absmax = 1.0

    for i, (case, attr_map) in enumerate(zip(cases, all_attr_maps)):
        # Raw positive and negative components — no gamma, no clipping
        neg_display = np.clip(-attr_map, 0, None)   # values ≥ 0
        pos_display = np.clip( attr_map, 0, None)   # values ≥ 0

        orig_uint8 = denormalize_image(
            case["image"], normalize_mean, normalize_std
        )
        # Overlay requires [-1, 1] for RGB blending — divide by group absmax only
        attr_for_overlay = attr_map / absmax
        overlay = _make_diverging_overlay(orig_uint8, attr_for_overlay, alpha=alpha, gamma=1.0)
        ...
        # Col 2 — Negative raw values (GnBu: white=0, dark blue=strong)
        axes[i, 1].imshow(neg_display, cmap="GnBu", vmin=0, vmax=absmax)
        if i == 0:
            axes[i, 1].set_title(col_headers[1], fontsize=9)
        axes[i, 1].axis("off")

        # Col 3 — Positive raw values (YlOrRd: white=0, dark red=strong)
        axes[i, 2].imshow(pos_display, cmap="YlOrRd", vmin=0, vmax=absmax)
        if i == 0:
            axes[i, 2].set_title(col_headers[2], fontsize=9)
        axes[i, 2].axis("off")

        # Col 4 — Mixed signed heatmap (RdBu_r: red=positive, blue=negative)
        axes[i, 3].imshow(attr_map, cmap="RdBu_r", vmin=-absmax, vmax=absmax)
        if i == 0:
    ...
    # Colorbars showing actual data range
    neg_ax = fig.add_axes([0.04, 0.025, 0.28, 0.012])
    neg_sm = ScalarMappable(cmap="GnBu", norm=mcolors.Normalize(vmin=0, vmax=absmax))
    neg_sm.set_array([])
    neg_cb = fig.colorbar(neg_sm, cax=neg_ax, orientation="horizontal")
    neg_cb.set_ticks([0, absmax])
    neg_cb.set_ticklabels([f"0", f"{absmax:.4f}  (strong blocker)"])
    neg_cb.ax.tick_params(labelsize=8)

    pos_ax = fig.add_axes([0.36, 0.025, 0.28, 0.012])
    pos_sm = ScalarMappable(cmap="YlOrRd", norm=mcolors.Normalize(vmin=0, vmax=absmax))
    pos_sm.set_array([])
    pos_cb = fig.colorbar(pos_sm, cax=pos_ax, orientation="horizontal")
    pos_cb.set_ticks([0, absmax])
    pos_cb.set_ticklabels([f"0", f"{absmax:.4f}  (strong booster)"])
    pos_cb.ax.tick_params(labelsize=8)

    mix_ax = fig.add_axes([0.68, 0.025, 0.28, 0.012])
    mix_sm = ScalarMappable(cmap="RdBu_r", norm=mcolors.Normalize(vmin=-absmax, vmax=absmax))
    mix_sm.set_array([])
    mix_cb = fig.colorbar(mix_sm, cax=mix_ax, orientation="horizontal")
    mix_cb.set_ticks([-absmax, 0, absmax])
    mix_cb.set_ticklabels([f"{-absmax:.4f}", "0", f"{absmax:.4f}"])
    mix_cb.ax.tick_params(labelsize=8)

    caption = (
        f"Raw signed IG attribution (np.sum across channels, no normalization).  "
        f"Group scale: ±{absmax:.4f}.  "
        f"Col 2: negative (blocks energy).  Col 3: positive (boosts energy).  "
        f"Col 4: signed heatmap.  Col 5: overlay (÷group absmax for blending only)."
    )
    fig.text(0.5, 0.003, caption, ha="center", fontsize=7, style="italic",
             wrap=True, color="0.3")
