"""
Grad-CAM visualization for the rubsheet_2to1 experiment.
Loads the trained rubsheet_2to1 model (latest by timestamp), runs inference
on the test set, selects the 10 best and 10 worst predictions, and generates
Grad-CAM heatmap grids for each group.

Note: rubsheet_2to1 images are 1068x534 (2:1 aspect ratio).
"""
import sys
import os
import glob
...
import config_rubsheet_2to1 as config
...
def find_latest_model(base_path):
    ...
    Model files are named like: rubsheet_2to1_model_2026-03-06_16-40.pth
    """
...
def plot_gradcam_grid(cases, cam_obj, device, title_prefix, save_path):
    ...
    Note: Images are 1068x534 (2:1 aspect ratio), so we use a wider figure.
    """
    n = len(cases)
    # Wider figure for 2:1 aspect ratio images
    ...
    plt.suptitle(f"{title_prefix}  (Regression Grad-CAM — Rubsheet 2-to-1)", fontsize=16)
