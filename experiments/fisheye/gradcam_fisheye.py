"""
Grad-CAM visualization for the fisheye experiment.
Loads the trained fisheye model, runs inference on the test set,
...
def plot_gradcam_grid(cases, cam_obj, device, title_prefix, save_path):
    ...
    For each case, plot 3 columns:
      Col 1 – Original Input Image:              the unmodified sky image.
      ...
      Col 3 – Contribution Applied to Original:  a red tint blended onto
              ...
              (clouds, sun, horizon) drove the prediction.
    """
    n = len(cases)
    ...
    plt.suptitle(f"{title_prefix}  (Regression Grad-CAM — Fisheye)", fontsize=16)
