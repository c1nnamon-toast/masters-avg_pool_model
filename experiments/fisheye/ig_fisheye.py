"""Integrated Gradients attribution analysis for the fisheye experiment.

Uses Captum's IntegratedGradients to explain SkyCNN's irradiance predictions
on the fisheye test set.
...
Run
---
    python experiments/fisheye/ig_fisheye.py
"""
...
# Number of integration steps. Higher = more accurate but slower.
# 50 is a good default; use 20 for quick tests, 200 for publication-quality.
N_STEPS = 50
...
def make_ig_attr_fn(ig, device, n_steps):
    """Return a callable that computes a raw (unnormalized) IG attribution map.

    The returned function:
    1. Sends the image to *device*.
    2. Calls ``IntegratedGradients.attribute`` with an all-zeros baseline and
       ``target=0`` (selecting the single scalar output of SkyCNN).
    3. Aggregates attribution across RGB channels using the L2 norm.
    4. Returns the raw 2-D map (NOT normalized to [0, 1]) so that the caller
       can inspect the true magnitude distribution before normalizing for
       display.
    ...
    def attr_fn(img_tensor):
        ...
        attr_map = np.sum(attr_np, axis=0)       # signed
        del attributions, inp, baseline
        return attr_map  # raw, unnormalized
...
def main():
    ...
    logger.info("=== Integrated Gradients — Fisheye ===")
