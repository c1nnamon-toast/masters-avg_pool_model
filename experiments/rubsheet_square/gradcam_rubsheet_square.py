"""
Grad-CAM visualization for the rubsheet_square experiment.
Loads the trained rubsheet_square model (latest by timestamp), runs inference
on the test set, selects the 10 best and 10 worst predictions, and generates
Grad-CAM heatmap grids for each group.

Note: rubsheet_square images are 534x534 (square, compressed from full aspect).
"""
import sys
import os
import glob
...
def find_latest_model(base_path):
    ...
    Model files are named like: rubsheet_square_model_2026-03-06_19-06.pth
    """
    ...
    if not model_files:
        # Fall back to base path if no timestamped versions exist
        if os.path.exists(base_path):
            return base_path
        raise FileNotFoundError(f"No model files found matching {pattern}")
    
    # Sort by timestamp in filename (format: YYYY-MM-DD_HH-MM)
...
def plot_gradcam_grid(cases, cam_obj, device, title_prefix, save_path):
    ...
    plt.suptitle(f"{title_prefix}  (Regression Grad-CAM — Rubsheet Square)", fontsize=16)
