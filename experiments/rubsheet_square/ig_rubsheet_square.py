"""Integrated Gradients attribution analysis for the rubsheet_square experiment.

Uses Captum's IntegratedGradients to explain SkyCNN's irradiance predictions
on the rubsheet_square test set (square-cropped polar-to-Cartesian unwrap).
...
Run
---
    python experiments/rubsheet_square/ig_rubsheet_square.py
"""
...
def make_ig_attr_fn(ig, device, n_steps):
    """Return a callable that computes a signed IG attribution map (not normalised)."""
...
def main():
    ...
    logger.info("=== Integrated Gradients — Rubsheet Square ===")
