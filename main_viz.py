...
OUTPUT_BASE = os.path.join(PROJECT_ROOT, "visualization", "avg_pool")
...
def find_latest_model(base_path):
    """Return the most recent timestamped .pth checkpoint (gap models excluded)."""
    ...
    model_files = [f for f in model_files if "gap" not in os.path.basename(f)]

    if not model_files:
        if os.path.exists(base_path):
            return base_path
        raise FileNotFoundError(f"No non-gap model files found matching {pattern}")
